...
*/
/**
 * Encode a user message's content into the DeepSeek multi-modal wire shape
 * using the Files API. Text becomes `{type:"text"}`, each image is uploaded
 * to POST /files to obtain a `file_id`, then referenced as `{type:"file",
 * file_id}` — the official recommended form that bypasses the 32 MiB inline
 * limit (a file_id image may be up to 64 MiB). Returns a bare string when
 * every block was text, preserving the single-string wire form for text-only
 * messages.
 */
async function serializeUserWithImages(blocks, imageFile) {
	const content = [];
...
/**
 * Upload one stored image to the DeepSeek Files API and return its file_id.
 * multipart/form-data: `file` (binary bytes + media type + filename) and
 * `purpose=user_data`; an expires_after is set so throwaway pasted images
 * are not retained forever.
 */
async function uploadImage(connection, apiKey, userId, stored, mediaType, signal) {
	const mediaSubtype = mediaType.split("/")[1] ?? "png";
	const filename = stored.ref?.filename ?? `paste.${mediaSubtype}`;
	const form = new FormData();
	form.append("file", new Blob([stored.data], { type: mediaType }), filename);
	...
	try {
		...
		throw new LlmError(`DeepSeek file upload to ${connection.baseURL}/files failed`, "TRANSPORT", { cause: error });
	}
	if (!response.ok) {
		let message = `DeepSeek file upload failed (HTTP ${response.status})`;
		try {
			const providerError = (await response.json()).error;
			if (providerError?.message) message = providerError.message;
		} catch {}
		throw new LlmError(message, "TRANSPORT", { status: response.status });
	}
	const json = await response.json();
	if (typeof json?.id !== "string" || json.id.length === 0) {
		throw new LlmError("DeepSeek file upload returned no file_id", "TRANSPORT");
	}
	return json.id;
}
 ...
 * user-role message, so a mixed user message contributes its text first and
 * its tool results as separate wire messages after.
 * For a model that declares image input, only user messages may carry an
 * image; a system or assistant image is rejected (the official endpoint
 * returns 400 for both). Without image support the existing text-only
 * rejection applies.
 * @param messages - the harness conversation, in order.
 * @returns the wire messages; order preserved, each tool result expanded into its own entry.
 */
async function serializeMessages(messages, supportsImage, imageFile) {
	const wire = [];
	for (const message of messages) {
		if (message.role === "system") {
			if (supportsImage && contentHasImage(message.content)) throw new LlmError("DeepSeek does not accept an image in a system message; move it to the user message", "UNSUPPORTED_CONTENT");
			wire.push({
				role: "system",
		...
		if (message.role === "assistant") {
			if (supportsImage && contentHasImage(message.content)) throw new LlmError("DeepSeek does not accept an image in an assistant message", "UNSUPPORTED_CONTENT");
		...
		const text = flattenText(message.content);
		if (supportsImage) {
			const content = await serializeUserWithImages(message.content.filter((block) => block.type !== "tool-result"), imageFile);
...
*/
async function serializeRequest(options, connection, imageFile, supportsImage) {
	...
	messages.push(...(await serializeMessages(options.messages, supportsImage, imageFile)));
...
var DeepSeekAdapter = class extends LlmAdapter {
	...
	async *request(options, signal, connection, apiKey, userId, onComment) {
		...
		const supportsImage = connection.models.some((m) => m.id === options.model && Array.isArray(m.inputModalities) && m.inputModalities.includes("image"));
		if (containsImage && !supportsImage) throw new LlmError(`DeepSeek model "${options.model}" does not support image input`, "UNSUPPORTED_CONTENT");
		const attachments = containsImage ? this.config.resolveAttachments?.() : void 0;
		if (containsImage && attachments === void 0) throw new LlmError("DeepSeek image input requires the durable attachment service", "UNSUPPORTED_CONTENT");
		let imageFile;
		if (containsImage) {
			const fileIdCache = new Map();
			imageFile = async (block) => {
				const hit = fileIdCache.get(block.attachment);
				if (hit) return hit;
				const stored = await attachments.readImage(block.attachment);
				const mediaType = stored.ref?.mediaType ?? block.attachment?.mediaType;
				if (mediaType === void 0 || !/^image\/(jpeg|png|gif|webp)/.test(mediaType)) {
					throw new LlmError(`DeepSeek does not support pasted media type ${mediaType ?? "(none declared)"}`, "UNSUPPORTED_CONTENT");
				}
				const fileId = await uploadImage(connection, apiKey, userId, stored, mediaType, signal);
				fileIdCache.set(block.attachment, fileId);
				return fileId;
			};
		}
		const body = await serializeRequest(options, connection, imageFile, supportsImage);
